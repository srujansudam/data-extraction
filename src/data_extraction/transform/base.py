...
class BaseTransformJob(ABC):
    ...
    def run(
        ...
        logger.info(
            "Transform started | run_id=%s job_name=%s source_system=%s target_table=%s "
        ...
        try:
            ...
            self.job_tracker.complete_job(
                ...
                rows_extracted=result.rows_read,
            ...
            logger.info(
                "Transform completed | status=success run_id=%s job_name=%s source_system=%s "
                "target_table=%s duration_seconds=%.3f rows_read=%s rows_inserted=%s "
            ...
            logger.error(
                "Transform failed | status=failure run_id=%s job_name=%s source_system=%s "
    ...
    def execute_transform(
        ...
        """Run the staging-to-final transform logic."""
