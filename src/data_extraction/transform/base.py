...
class BaseTransformJob(ABC):
    ...
    def run(
        ...
        logger.info("Starting transform job: %s", self.job_name)
        ...
        try:
            ...
            logger.info(
                "Completed transform job: %s | read=%s inserted=%s updated=%s rejected=%s",
                self.job_name,
                result.rows_read,
            ...
            logger.exception("Failed transform job: %s", self.job_name)
            raise
